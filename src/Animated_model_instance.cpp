...
static int animated_model_instance_cast(lua_State *L)
{
	shared_ptr<Scenenode> scenenode = check_scenenode(L, 1);
	shared_ptr<Animated_model_instance> animated_model_instance(scenenode);
	if(animated_model_instance.get())
		push_animated_model_instance(L, animated_model_instance);
	else
		lua_pushnil(L);
	return 1;
}
...
static const luaL_reg animated_model_instance_methods[] = {
	...
	{"cast", animated_model_instance_cast},
